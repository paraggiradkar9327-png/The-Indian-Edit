...
import { UploadCloud, CheckCircle2, ArrowRight, ShieldCheck, Sparkles, Image as ImageIcon, FileText } from 'lucide-react';
import { sound } from '../../utils/audio';

export const UploadScreen: React.FC = () => {
  const { state, setScreenshotUploaded, navigateTo } = useGame();

  const [previewUrl, setPreviewUrl] = useState<string | null>(state.uploadedScreenshotUrl);
  ...
  return (
    <div className="max-w-xl mx-auto py-8 px-4 sm:px-6 animate-fade-in">
      <div className="gold-card p-6 sm:p-8 text-center">
        ...
        <span className="text-xs font-bold tracking-widest text-[#d4af37] uppercase">Privilege Validation</span>
        ...
        <p className="text-xs text-[#a69383] mt-2 max-w-sm mx-auto">
          Upload a screenshot of your Instagram story or post tagging <strong>@TheIndianEdit</strong> to unlock the royal scratch card.
        </p>
        ...
        {verified && (
          <div className="mt-4 p-4 rounded-xl bg-gradient-to-r from-green-950/40 to-[#22160f] border border-green-500/60 text-center space-y-2 animate-fade-in">
            ...
            <p className="text-[11px] text-[#warm-beige]">
              Your VIP privilege code is authenticated. The Gold Scratch Card is now ready to reveal.
            </p>
        ...
        <div className="mt-6 pt-4 border-t border-[#3d261a] text-[11px] text-[#8c7766] flex items-center justify-center gap-1.5">
          ...
          <span>Official verification powered by The Indian Edit Brand Protocol</span>
        </div>
